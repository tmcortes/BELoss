...
from cirtorch.layers.pooling import MAC, SPoC, GeM, GeMmp, RMAC
...
class ImageRetrievalNet(nn.Module):
    ...
    def cuda(self):
        super(ImageRetrievalNet, self).cuda()
        self.meta['Lw']['P'] = self.meta['Lw']['P'].cuda()
        self.meta['Lw']['m'] = self.meta['Lw']['m'].cuda()
        return self
    
    def cpu(self):
        super(ImageRetrievalNet, self).cpu()
        self.meta['Lw']['P'] = self.meta['Lw']['P'].cpu()
        self.meta['Lw']['m'] = self.meta['Lw']['m'].cpu()
        return self
...
def init_network(model='resnet101', pooling='gem', whitening=False, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225], pretrained=True):
    ...
    P = torch.eye(dim)
    m = torch.zeros(dim).unsqueeze(1)
    Lw = {'P': P, 'm': m}
    meta = {'architecture':model, 'pooling':pooling, 'whitening':whitening, 'outputdim':dim, 'mean':mean, 'std':std, 'Lw': Lw}
...
def extract_vectors(net, images, image_size, transform, bbxs=None, ms=[1], msp=1, print_freq=1000):
    ...
    for i, input_ in enumerate(loader):
        ...
        input_var = Variable(input_.cuda())

...
class Rpool(nn.Module):
    ...
    def __init__(self, rpool, whiten=None, L=4, eps=1e-6):
    ...
    def forward(self, x, aggregate=False):
        # features -> roipool
...
class KLW(nn.Module):
    ...
    def __init__(self, r):
        super(KLW,self).__init__()
        self.r = r
        self.alphas = Parameter(torch.ones(1,1,r,1))
        
    def forward(self, x):
        #print(x.shape)
        #return x.squeeze(-1).squeeze(-1)
    
        self.alphas.data = self.alphas.data.clamp(0)
        return LF.klw(x, alphas=self.alphas)
        
    def __repr__(self):
        return self.__class__.__name__ + '(' + 'alphas=' + ')'
